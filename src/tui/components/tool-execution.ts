...
/** Color-coded status pill for tool execution state. */
function statusPill(isPartial: boolean, isError: boolean): string {
  if (isPartial) return theme.warning("⏳ Running");
  if (isError) return theme.error("❌ Failed");
...
export class ToolExecutionComponent extends Container {
  ...
  private refresh() {
    ...
    // Collapsible indicator + status pill
    const expandIcon = this.expanded ? "▾" : "▸";
    const pill = statusPill(this.isPartial, this.isError);
    const title = `${expandIcon} ${display.emoji} ${theme.bold(theme.toolTitle(display.label))} ${pill}`;
