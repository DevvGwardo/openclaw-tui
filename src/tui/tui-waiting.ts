...
/** Get the braille spinner character for a given tick. */
export function spinnerFrame(tick: number): string {
  return BRAILLE_SPINNER[tick % BRAILLE_SPINNER.length] ?? BRAILLE_SPINNER[0]!;
}
...
export function buildWaitingStatusMessage(params: {
  ...
  const spinner = spinnerFrame(params.tick);
  const phrase = pickWaitingPhrase(params.tick, params.phrases);
  const cute = shimmerText(params.theme, `${phrase}…`, params.tick);
  return `${params.theme.accentSoft(spinner)} ${cute} • ${params.elapsed} | ${params.connectionStatus}`;
}
