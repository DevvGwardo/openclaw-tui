...
import { editorTheme, theme, getCurrentPalette } from "./theme/theme.js";
...
export async function runTui(opts: TuiOptions) {
  ...
  // Startup banner — gradient "OpenClaw" text
  const p = getCurrentPalette();
  const banner = gradient("🦞 OpenClaw", p.primary, p.tertiary);
  const bannerText = new Text(`${banner} ${theme.dim("─ terminal ui")}`, 1, 0);
  ...
  const updateHeader = () => {
    ...
    const url = theme.dim(client.connection.url);
    const agentPart = theme.accent(agentLabel);
    const sessionPart = theme.accentSoft(sessionLabel);
    header.setText(
      `${url} ${theme.dim("│")} ${theme.dim("agent")} ${agentPart} ${theme.dim("│")} ${theme.dim("session")} ${sessionPart}`,
    );
  };
  ...
  const updateFooter = () => {
    ...
    // Feed the new status bar with structured data
    statusBar.update({
      sessionLabel: sessionKeyLabel,
      modelLabel,
      agentLabel,
      inputTokens: sessionInfo.inputTokens,
